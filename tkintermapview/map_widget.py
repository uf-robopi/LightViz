...
from .canvas_dot import CanvasDot
from .canvas_light_field_map import CanvasLightFieldMap
...
class TkinterMapView(tkinter.Frame):
    ...
    def set_address(self, address_string: str, marker: bool = False, text: str = None, **kwargs) -> CanvasPositionMarker:
        ...
        result = geocoder.osm(address_string)
        # result = [{"place_id":294466527,"licence":"Data © OpenStreetMap contributors, ODbL 1.0. http://osm.org/copyright","osm_type":"relation","osm_id":162050,"lat":"27.7567667","lon":"-81.4639835","category":"boundary","type":"administrative","place_rank":8,"importance":0.7512139727980621,"addresstype":"state","name":"Florida","display_name":"Florida, United States","address":{"state":"Florida","ISO3166-2-lvl4":"US-FL","country":"United States","country_code":"us"},"boundingbox":["24.3963080","31.0007620","-87.6348960","-79.9743060"]}]
        # print(str(result.latlng))
        if result.ok:
    ...
    def set_dot(self, deg_x: float, deg_y: float, val: float, text: str = None, **kwargs) -> CanvasDot:
        dot = CanvasDot(self, (deg_x, deg_y), val, text=text, **kwargs)
        dot.draw()
        self.canvas_marker_list.append(dot)
        return dot

    def set_light_field_map(self, deg_x: float, deg_y: float, val: float, text: str = None, **kwargs) -> CanvasLightFieldMap:
        lightFM = CanvasLightFieldMap(self, (deg_x, deg_y), val, text=text, **kwargs)
        lightFM.draw()
        self.canvas_marker_list.append(lightFM)
        return lightFM
    ...
    def delete(self, map_object: any):
        if isinstance(map_object, (CanvasPath, CanvasPositionMarker, CanvasPolygon, CanvasDot)):
            map_object.delete()
